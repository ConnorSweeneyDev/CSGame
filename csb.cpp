       ...
       else
       {
         ...
         if (!hitbox_names.empty())
         {
           ...
           for (const auto &hitbox_name : hitbox_names)
             result += std::format("        const cse::name {};\n", hitbox_name);
           result += "      };\n";
           result += "    }\n";
       ...
       for (const auto &animation : animations)
       {
         ...
         for (unsigned int frame_index{start_frame - 1}; frame_index < end_frame; ++frame_index)
         {
           if (frame_count < animation_hitboxes.size() && !animation_hitboxes[frame_count].empty())
           {
             ...
             animations_result +=
               std::format("  static constexpr std::array<const std::pair<cse::name, cse::rectangle>, {}> "
             ...
             {
               animations_result +=
                 std::format("{{cse::name(\"{}\"), {{{:#g}, {:#g}, {:#g}, {:#g}}}}}", name + "." + hitbox_name,
       ...
       for (const auto &[file, name, data] : files)
       {
         ...
         else
         {
           ...
           if (!hitbox_names.empty())
           {
             ...
             {
               result += std::format("cse::name(\"{}\")", name + "." + hitbox_names[i]);
