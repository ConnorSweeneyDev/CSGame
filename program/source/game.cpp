...
namespace csg
{
  ...
  void game::pre_simulate(const double)
  {
    ...
    if (equal(graphics.previous.aspect_ratio.value, 16.0 / 9.0) && equal(graphics.active.aspect_ratio.value, 4.0 / 3.0))
    ...
    if (!equal(graphics.previous.frame_rate, graphics.active.frame_rate))
      cse::print<COUT>("Frame rate changed from {} to {}\n", graphics.previous.frame_rate, graphics.active.frame_rate);
